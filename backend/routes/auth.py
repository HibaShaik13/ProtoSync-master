...
def signup():
    ...
    workspaceName = data.get('workspaceName') or 'My Workspace'
    ...
    # Resolve JWT decoding to string if it is bytes (PyJWT < 2.0 generates string natively, PyJWT >= 2.0 does too. But for robust handling:
...
def login():
    ...
    if not user or not bcrypt.checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
        return jsonify({"error": "Invalid email or password associated with this cluster gateway"}), 401
    ...
    return jsonify({
        ...
        "token": token,
        "user": {
            "fullName": user.get("fullName") or user.get("name", "User"),
            "email": user["email"],
            "workspaceName": user.get("workspaceName", "My Workspace"),
            "token": token
        },
