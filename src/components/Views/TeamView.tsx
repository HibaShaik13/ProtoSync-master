...
export default function TeamView({ user, showNotification }: TeamViewProps) {
  ...
  React.useEffect(() => {
    if (user && user.email) {
      fetch(getApiUrl(`/api/user/workspace-data?email=${encodeURIComponent(user.email)}`), {
        headers: {
    ...
    fetch(getApiUrl(`/api/user/workspace-data?email=${encodeURIComponent(user.email)}`), {
      ...
      .then(result => {
        if (result.success && result.workspaceData) {
          ...
          fetch(getApiUrl('/api/user/workspace-data'), {
            method: 'POST',
            headers: {
  ...
  const handleRequestOtpSubmit = (e: React.FormEvent) => {
    ...
    fetch(getApiUrl('/api/team/send-otp'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  const handleVerifyOtpSubmit = (e: React.FormEvent) => {
    ...
    fetch(getApiUrl('/api/team/verify-otp'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
